# API Server
- [Control Plane](Control%20Plane.md)의 핵심이다
...
- 권한 부여 (Authorization)
	...
	- [Kubernetes](Kubernetes.md)는 [RBAC](RBAC.md), [ABAC](ABAC.md) 등 다양한 권한 부여 방식을 지원한다.
- 승인(Admission)
	- API Server는 사용자 요청을 처리하기 전에 승이 단계를 거친다.
	- 승인 컨트롤러는 클러스터에 영향을 미치기 전에 해당 요청이 적절한지 검사하고, 필요시 해당 요청을 거부, 수정할 수있다.
- API Object 저장 및 관리
	- API Server 는 클러스터의 리소스를 [etcd](etcd.md)라는 분산 key-value 저장소에 저장하고 관리한다.
	- [etcd](etcd.md)는 클러스터의 상태를 일관성 있게 유지하고 각 컴포넌트 사이에서 공유하기 위한 데이터 저장소이다.
